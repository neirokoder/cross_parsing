...
def main():
    name = sys.argv[1] if len(sys.argv) > 1 else '2-020101-174-12'
    raw = os.path.join(ROOT, 'work', 'etalon_build', 'raw')
    os.makedirs(raw, exist_ok=True)
    pdf = fitz.open(os.path.join(ROOT, 'data', 'pdf', f'{name}.pdf'))
    ...
    html_dir = os.path.join(ROOT, 'data', 'html', name)
    ...
    print('prep done', name, pdf.page_count, 'pages;', len(meta.get('formula_bboxes', {})) if isinstance(meta, dict) else meta)
